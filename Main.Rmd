...
shinyWidgets::selectizeGroupUI(
  ...
  params = list(
    ...
    `State Name` = list(inputId = "StateName", title = "State:"),
    `County Name` = list(inputId = "CountyName", title = "County:")
  ),
  label = "Filters",
...
filter_covid_19 <- shiny::callModule(
  ...
  id = "geoFilters",
  data = covid_19,
  vars = c("Level", "State Name", "County Name")
)
...
DT::renderDataTable({
  filter_covid_19() %>% 
    prep_output_data(!!input$metric, log_axis = "No", latest = TRUE) %>% 
    dplyr::select(Date, Value, `State Name`, `County Name`)
})
